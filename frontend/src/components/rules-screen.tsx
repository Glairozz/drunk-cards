    ...
    >
      <div className="mb-6 sm:mb-8">
        <div className="relative inline-block mb-4">
          <motion.span
            className="text-4xl sm:text-[3.5rem] relative z-10 inline-block"
          ...
          <motion.div
            className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[80px] sm:w-[100px] h-[80px] sm:h-[100px] border-2 border-white/10 rounded-full"
            animate={{ scale: [1, 1.3, 1], opacity: [0.5, 0, 0.5] }}
            transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
          />
        </div>
        <h2 className="font-heading text-2xl sm:text-[1.75rem] font-bold text-white -tracking-wide">How to Play</h2>
      </div>

      <div className="w-full mb-6 sm:mb-8 flex flex-col gap-3">
        {rules.map((rule, i) => (
          <motion.div
            key={rule.num}
          ...
          >
            ...
            <span className="text-xl sm:text-2xl w-9 sm:w-10 h-9 sm:h-10 flex items-center justify-center bg-[rgba(255,107,53,0.1)] rounded-xl shrink-0">
              {rule.icon}
            </span>
            <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
              <span className="font-heading text-xs sm:text-sm font-extrabold text-[#ff6b35] bg-[rgba(255,107,53,0.1)] px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-md shrink-0">
                {rule.num}
              </span>
              <p className="text-xs sm:text-sm text-white/80 leading-relaxed">{rule.text}</p>
            </div>
          </motion.div>
        ))}
      </div>
